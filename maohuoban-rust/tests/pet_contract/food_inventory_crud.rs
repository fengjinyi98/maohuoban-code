...
#[tokio::test]
async fn food_inventory_consume_one_closes_ordered_package_cycles() {
    let app = maohuoban_rust::test_support::spawn_auth_test_app().await;
    app.reset().await;
    let user_id = login_user_id(&app, "13800139042").await;
    ...
    let first_response = app
        ...
        .oneshot(empty_request(
            "POST",
            &format!("/api/v1/food-inventory/items/{item_id}/consume-one"),
            Some(&user_id),
        ))
        .await
        .expect("consume first package");
    assert_eq!(first_response.status(), StatusCode::OK);
    let first_body = response_json(first_response).await;
    assert_eq!(first_body["code"], "food_inventory.item_consumed");
    assert_eq!(first_body["message"], "已吃完 高爷家益生菌猫粮 1 袋");
    assert_eq!(first_body["data"]["item"]["quantity"], 1);
    assert_eq!(first_body["data"]["item"]["inventory_status"], "in_use");
    assert_eq!(first_body["data"]["consumption_cycle"]["sequence_no"], 1);
    assert_eq!(
        first_body["data"]["consumption_cycle"]["package_weight_grams"],
        1500
    );
    assert_eq!(
        first_body["data"]["consumption_cycle"]["package_unit"],
        "袋"
    );
    ...
    let second_response = app
        ...
        .await
        .expect("consume second package");
    assert_eq!(second_response.status(), StatusCode::OK);
    let second_body = response_json(second_response).await;
    assert_eq!(second_body["code"], "food_inventory.item_consumed");
    assert_eq!(second_body["message"], "已吃完 高爷家益生菌猫粮 1 袋");
    assert_eq!(second_body["data"]["item"]["quantity"], 0);
    assert_eq!(second_body["data"]["item"]["inventory_status"], "depleted");
    assert_eq!(second_body["data"]["consumption_cycle"]["sequence_no"], 2);
    ...
    let third_response = app
        ...
        .await
        .expect("consume empty item");
    assert_eq!(third_response.status(), StatusCode::BAD_REQUEST);
    let third_body = response_json(third_response).await;
    assert_eq!(third_body["message"], "当前库存已经没有可确认消耗的数量");
}
