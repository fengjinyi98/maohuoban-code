        ...
        then.status(200)
            ...
            .body(
                "data: {\"choices\":[{\"delta\":{\"tool_calls\":[{\"id\":\"call_save_plan\",\"function\":{\"name\":\"save_abnormal_episode_followup_plan\",\"arguments\":\"{\\\"due_at\\\":\\\""
                    .to_owned()
                    + planned_due_at
                    + "\\\",\\\"message_title\\\":\\\"毛球稍后再确认\\\",\\\"message_body\\\":\\\"继续确认便便、精神和食欲是否好转。\\\",\\\"rationale\\\":\\\"用户仍在异常追踪中，需要稍后复查。\\\",\\\"time_decision\\\":{\\\"now_at\\\":\\\"2026-07-05T08:00:00Z\\\",\\\"occurred_at\\\":\\\"2026-07-05T00:10:00Z\\\",\\\"episode_started_at\\\":\\\"2026-07-05T00:10:00Z\\\",\\\"last_observed_at\\\":\\\"2026-07-05T00:10:00Z\\\",\\\"elapsed_minutes\\\":470,\\\"attention_timing\\\":\\\"scheduled_later\\\",\\\"staleness_assessment\\\":{\\\"basis\\\":\\\"now_at - max(occurred_at,last_observed_at)\\\",\\\"staleness_minutes\\\":470,\\\"reason\\\":\\\"异常追踪中仍缺少最新状态\\\"},\\\"identity_context\\\":{\\\"species\\\":\\\"cat\\\",\\\"birthday\\\":\\\"2025-01-01\\\",\\\"world_days\\\":185},\\\"selected_due_at\\\":\\\""
                    + planned_due_at
                    + "\\\",\\\"delay_minutes\\\":630,\\\"urgency_window\\\":\\\"short_delay\\\",\\\"reason\\\":\\\"仍在追踪中，短延迟复查\\\",\\\"time_tool_used\\\":true},\\\"recommended_actions\\\":[\\\"update_observation\\\",\\\"chat_with_agent\\\"]}\"}}]}}]}\n\n\
                 data: {\"choices\":[{\"finish_reason\":\"tool_calls\"}],\"usage\":{\"prompt_tokens\":8,\"completion_tokens\":2,\"total_tokens\":10}}\n\n\
                 data: [DONE]\n\n",
            );
    });
        ...
        then.status(200)
            ...
            .body(
                "data: {\"choices\":[{\"delta\":{\"tool_calls\":[{\"id\":\"call_save_branch_plan\",\"function\":{\"name\":\"save_abnormal_episode_followup_plan\",\"arguments\":\"{\\\"due_at\\\":\\\""
                    .to_owned()
                    + due_at
                    + "\\\",\\\"message_title\\\":\\\"毛球建议尽快处理\\\",\\\"message_body\\\":\\\"异常有加重迹象，建议联系医院并继续补充状态。\\\",\\\"rationale\\\":\\\"用户反馈加重，需要提供就医入口并持续追踪。\\\",\\\"time_decision\\\":{\\\"now_at\\\":\\\"2026-07-05T18:00:00Z\\\",\\\"occurred_at\\\":\\\"2026-07-05T00:10:00Z\\\",\\\"episode_started_at\\\":\\\"2026-07-05T00:10:00Z\\\",\\\"last_observed_at\\\":\\\"2026-07-05T00:10:00Z\\\",\\\"elapsed_minutes\\\":1070,\\\"attention_timing\\\":\\\"now_or_soon\\\",\\\"staleness_assessment\\\":{\\\"basis\\\":\\\"now_at - max(occurred_at,last_observed_at)\\\",\\\"staleness_minutes\\\":1070,\\\"reason\\\":\\\"加重反馈后需要尽快补充最新状态\\\"},\\\"identity_context\\\":{\\\"species\\\":\\\"cat\\\",\\\"birthday\\\":\\\"2025-01-01\\\",\\\"world_days\\\":185},\\\"selected_due_at\\\":\\\""
                    + due_at
...
#[tokio::test]
#[allow(clippy::too_many_lines)]
async fn abnormal_followup_plan_rejects_body_with_conflicting_local_event_date() {
    let _guard = diagnostics_test_lock().lock_owned().await;
    let server = MockServer::start();
    let mut config = maohuoban_rust::BackendConfig::local_test();
    config.ai_llm_provider_config = maohuoban_ai_infrastructure::provider::OpenAiCompatibleConfig {
    ...
    let planned_due_at = "2026-07-06T12:00:00Z";
    let save_mock = server.mock(|when, then| {
        when.method(httpmock::Method::POST)
            .path("/v1/chat/completions")
            .header("authorization", "Bearer contract-api-key")
        ...
        then.status(200)
            .header("content-type", "text/event-stream")
            .body(
                "data: {\"choices\":[{\"delta\":{\"tool_calls\":[{\"id\":\"call_save_bad_date_plan\",\"function\":{\"name\":\"save_abnormal_episode_followup_plan\",\"arguments\":\"{\\\"due_at\\\":\\\""
                    .to_owned()
                    + planned_due_at
                    + "\\\",\\\"message_title\\\":\\\"馒头情况追踪提醒\\\",\\\"message_body\\\":\\\"距离昨天（7/5）发现馒头出现明显异常已过去约 8 小时，现在情况怎么样了？\\\",\\\"rationale\\\":\\\"严重异常需要追踪。\\\",\\\"time_decision\\\":{\\\"now_at\\\":\\\"2026-07-06T07:23:28Z\\\",\\\"occurred_at\\\":\\\"2026-07-05T23:20:00Z\\\",\\\"episode_started_at\\\":\\\"2026-07-05T23:20:00Z\\\",\\\"last_observed_at\\\":null,\\\"elapsed_minutes\\\":483,\\\"attention_timing\\\":\\\"scheduled_later\\\",\\\"staleness_assessment\\\":{\\\"basis\\\":\\\"now_at - max(occurred_at,last_observed_at)\\\",\\\"staleness_minutes\\\":483,\\\"reason\\\":\\\"严重异常缺少最新状态\\\"},\\\"identity_context\\\":{\\\"species\\\":\\\"cat\\\",\\\"birthday\\\":\\\"2025-01-01\\\",\\\"world_days\\\":551},\\\"selected_due_at\\\":\\\""
                    + planned_due_at
                    + "\\\",\\\"delay_minutes\\\":277,\\\"urgency_window\\\":\\\"high\\\",\\\"reason\\\":\\\"严重异常需要追踪\\\",\\\"time_tool_used\\\":false},\\\"recommended_actions\\\":[\\\"update_observation\\\",\\\"chat_with_agent\\\"]}\"}}]}}]}\n\n\
                 data: {\"choices\":[{\"finish_reason\":\"tool_calls\"}],\"usage\":{\"prompt_tokens\":8,\"completion_tokens\":2,\"total_tokens\":10}}\n\n\
                 data: [DONE]\n\n",
            );
    });
    let answer_mock = server.mock(|when, then| {
        when.method(httpmock::Method::POST)
            ...
            .header("authorization", "Bearer contract-api-key")
            .body_contains("\"stream\":true")
            .body_contains("\"tool_call_id\":\"call_save_bad_date_plan\"");
        then.status(200)
            .header("content-type", "text/event-stream")
            .body(
    ...
    let response = app
        ...
        .await
        .expect("send abnormal followup invalid date plan");
