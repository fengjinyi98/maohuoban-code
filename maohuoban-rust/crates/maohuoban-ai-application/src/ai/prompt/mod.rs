...
impl AiPromptBuilder {
    ...
    fn build_system_prompt() -> String {
        ...
        prompt.push_str("- 公共养宠咨询、毛伙伴产品帮助和助手身份问题，可以在没有私域事实包时基于通用知识和本轮能力边界回答。\n");
        prompt.push_str("- 涉及用户自己宠物档案、记录、提醒、饮食等私域事实时，只能基于提供的事实包或工具结果回答。\n");
        prompt.push_str("- 强事实（标注为 confirmed/strong）可以直接引用。\n");
        prompt.push_str("- 弱线索（标注为 weak/hint）是待确认信息，不能表达为已发生的事实，只能提示用户确认。\n");
        prompt.push_str(
            "- 如果私域事实包或工具结果中没有相关信息，请如实告知用户暂时无法获取记录，不要编造。\n",
        );
