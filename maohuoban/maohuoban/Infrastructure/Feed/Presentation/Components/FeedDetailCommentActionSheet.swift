...
// FeedDetailCommentActionSheet Feed 详情评论操作面板
// 核心职责：
// - 承接评论或留言长按后的底部操作
// - 根据评论所有权展示删除或举报危险操作
struct FeedDetailCommentActionSheet: View {
    ...
    private func actionRow(
        ...
        .buttonStyle(FeedDetailCommentActionSheetRowButtonStyle())
    }
}

// FeedDetailCommentActionSheetOverlay Feed 详情评论操作遮罩
// 核心职责：
// - 提供底部评论操作面板的呈现和退出动画
// - 通过遮罩点击关闭当前操作上下文
struct FeedDetailCommentActionSheetOverlay: View {
    let comment: FeedComment
    let onDismiss: () -> Void
    let onReply: () -> Void
    let onCopy: () -> Void
    ...
    var body: some View {
        ...
        .animation(FeedDetailLayout.commentComposerAnimation, value: comment.id)
    }
}

// FeedDetailCommentActionSheetRowButtonStyle 评论操作行按钮样式
// 核心职责：
// - 为操作面板的整行按钮提供按压反馈
// - 保证点击命中范围覆盖整行
private struct FeedDetailCommentActionSheetRowButtonStyle: ButtonStyle {
