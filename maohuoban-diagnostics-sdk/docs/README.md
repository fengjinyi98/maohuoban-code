...
| Rust | `diagnostics.capture_runtime_snapshot(...)` | `process_id`、`process_name`、`os`、`arch`、`uptime_ms`、`dropped_event_count`、`last_storage_error` |

运行时快照会作为 `kind=performance`、`severity=info` 的标准事件写入，用于启动、卡顿、网络异常和错误链前后的环境记录。Swift 和 Rust 在发生事件落盘失败后会写入 `dropped_event_count` 和 `last_storage_error`，用于判断诊断数据本身是否丢失。
