...
| 节点 | 当前状态 | 需要补齐 |
|---|---|---|
| 1. 正常事实记录 | 便便正常、精神不错、食欲正常、喂食等记录已具备部分闭环 | 确认这些记录稳定进入时间线和 Agent 可读事实上下文 |
| 2. 创建异常记录 | 异常记录已有前端和部分后端链路 | 确认异常事件结构、照片附件、时间线和详情读取稳定 |
| 3. abnormal_episode | 已有异常追踪目标文档和后端模型方向 | 补齐 episode 创建、状态、详情读取和事件关联 |
| 4. 延迟追踪计划 | 目前仍偏产品讨论 | 需要 follow-up plan / attention hint / reminder trigger 的最小规则 |
| 5. 异常更新入口 | 当前需要明确入口形态 | 补齐结构化异常更新 Sheet，必要时接 Agent 聊天入口 |
| 6. 异常更新落库 | 待实现 | 写入 abnormal_followup，并更新 episode 状态 |
| 7. Agent 追问 | Agent 能力需与事实上下文打通 | 只能基于结构化事实和用户文字追问，不解析照片 |
| 8. 好转分支 | 待实现 | 降低追踪频率、确认恢复、关闭 episode、保留时间线事实 |
| 9. 持续分支 | 待实现 | 按节奏继续提醒，追问缺失事实，超过窗口进入就医建议 |
| 10. 加重分支 | 待实现 | 风险条件、就医建议、预约合作医院入口 |
| 11. 诊前资料包 | 待设计/实现 | 生成快照，包含事实、文字、附件列表和 Agent 文字摘要 |
| 12. HIS 接收资料包 | HIS Web 目前是占位/Mock 阶段 | 医生工作台展示诊前资料包 |
| 13. HIS 诊疗与发布 | 待实现 | 接诊、诊断、处方、发布健康档案 |
| 14. App 病历回流 | 当前 App 病历仍是 mock 语义 | 接收 HIS 发布版病历，展示来源和详情 |
| 15. 病历后续追踪 | 待实现 | 用药反馈、恢复情况、复诊提醒进入后续闭环 |
