...
当前 Web HIS 已完成真实后端和开发库基线接入：登录、医院上下文、合作医院院区、今日工作台预约队列、患者列表和患者详情都来自 Rust 后端 `/api/v1/his/*` 与开发库真实表。Web HIS 运行路径已移除 MSW、fixture 和 `/api/mock` 数据源。开发库保留用于闭环验证的 HIS 租户、合作医院和医生账号；临时插入的验证预约 `55555555-5555-4555-8555-555555555555` 已删除。App 预约医院已接入真实 SameCity 后端，预约成功后进入 HIS 今日工作台；App 取消预约会把预约状态更新为 `cancelled`，HIS 工作台过滤已取消预约。
...
| 7. Agent 追问 | 异常事件/episode 事实读取已打通；异常轻提醒进入 Agent 会话的上下文、同 episode 会话复用和同 Agent 上下文复用已打通；到期首条主动追问会写入同一后台追踪会话，用户点击 `问问毛球` 后才激活为可见聊天；iOS 进入异常追踪会话会恢复同 session 消息并对最新已完成 assistant 追问展示“毛球主动追问”；用户确认写回 `symptom_followup` 已打通；普通 Agent 聊天内模型可通过 `prepare_pet_abnormal_symptom_creation` 准备异常创建确认卡，用户授权后后端创建 `health/abnormal_symptom`、`abnormal_episode` 和初始 `planning`，并把当前聊天 session 绑定为同一异常追踪上下文；确认写入后同一会话继续由 Agent 回复，回复通过 SSE 返回并由 finalizer 持久化到 `ai_messages`；确认任务流式边界已收敛，模型同轮先输出自由文本再调用确认工具时，前端只收到授权卡，不先展示 `answer_delta`；异常追踪 workflow skill 已能进入模型上下文；模型规划结果可通过受控 tool 保存；后台 planner 已能读取多源事实并生成下一轮轻提醒计划 | 只能基于结构化事实和用户文字追问，不解析照片；写入前用户可见主结果是授权卡 | 已验证能力具备 |
...
| 10. 加重分支 | 已具备 `condition_change=worsened` 后进入 `escalated`，并生成带 `book_clinic` 动作的追踪 planning；Agent planning tool schema 和 service 白名单允许保存分支动作；App 预约合作医院入口已接入真实 SameCity/HIS 队列 | 后续把加重分支动作直接串联到诊前资料包和预约入口 | 已验证能力具备 |
| 11. 诊前资料包 | 待设计/实现 | 生成快照，包含事实、文字、附件列表和 Agent 文字摘要 | 未开始 |
| 12. HIS 接收资料包 | Web HIS 已接入真实后端和开发库：真实员工登录、HIS 租户/合作医院上下文、今日工作台预约队列、患者列表和患者详情读取已具备；App 预约医院会创建真实预约并进入 HIS 今日工作台；App 取消预约会更新 `cancelled` 并从 HIS 工作台过滤；预约成功/取消成功 Toast 使用后端 `message`；预约合作医院页面开发验证阶段固定显示“毛伙伴验证闭环医院” | 预约单携带诊前资料包；医生工作台展示诊前资料包 | 部分具备，诊前资料包未开始 |
...
本节点当前覆盖范围是：异常记录创建、异常事件结构、episode 关联、首页轻提示、时间线/详情读取、照片附件字段闭环，以及 Agent 对异常事件事实的只读引用。异常 episode 的完整状态页、更细就医预约入口和诊前资料包仍由后续节点继续收敛。
...
本节点当前覆盖范围是：episode 父对象、状态、父子事件聚合、附件存在性、Agent 只读事实工具、合同测试和诊断包验证。后续更细状态展示、真实预约入口和诊前资料包仍在后续节点实现。
...
| 能力 | 当前结论 | 证据 |
|---|---|---|
| 追踪计划模型 | 已具备 | 迁移 `0048_agent_proactive_followups.sql` 新增 `agent_proactive_followups`，并在 `abnormal_episodes` 上保存 `next_followup_due_at/last_followup_plan_id`；迁移 `0051_ai_tool_access_payload_audit.sql` 补充 `planning_decision` |
| 异常创建触发计划 | 已具备默认计划 | 创建 `health/abnormal_symptom` 时事务写入初始 `planning` followup；该 due_at 是 planner fallback touchpoint，不是模型最终观察窗口；合同测试 `abnormal_creation_creates_initial_agent_followup_plan`、`abnormal_creation_initial_followup_due_is_planner_fallback` 已通过 |
...
| 用户追加后的重规划 | 已具备分支计划 | 写入 `symptom_followup` 后后端事务创建下一轮 `planning` followup，并回写 episode `next_followup_due_at/last_followup_plan_id`；`improved/unchanged/worsened` 分别进入恢复确认、继续追踪、就医入口分支；合同测试 `symptom_followup_creates_next_agent_followup_plan`、`improved_symptom_followup_marks_episode_recovering_and_plans_recovery_confirmation`、`worsened_symptom_followup_marks_episode_escalated_and_plans_clinic_action` 已通过 |
| 终止追踪投影清理 | 已具备 | 写入 `abnormal_recovery` 后清空 `abnormal_episodes.next_followup_due_at/last_followup_plan_id`，父异常删除后关闭 episode 并清空追踪投影，同时取消/resolve 计划和轻提醒；合同测试 `abnormal_recovery_clears_agent_followup_projection`、`deleting_parent_abnormal_event_clears_agent_followup_projection` 已通过 |
...
| Agent 确认卡流式边界 | 已具备 | Runtime SSE Projector 在模型调用期间缓冲 `MessageDelta`；如果同轮模型最终以 `tool_calls` 进入 `prepare_pet_observation_write` 或其他确认任务，丢弃本轮自由文本并只投影 `confirmation_task`；如果是普通回答完成，再释放可见文本。合同测试 `ai_chat_stream_suppresses_model_delta_before_confirmation_task` 已通过，覆盖“先输出文本再出现授权卡”的回归 |
...
| Agent 聊天创建异常 | 用户在普通 Agent 会话中描述异常时，模型可生成待确认异常创建草稿；用户授权后后端创建异常事件、episode 和初始 planning，并把当前 session 绑定为该 episode 的追踪上下文 |
...
普通 Agent 聊天创建异常走同一条后半链路：

```text
用户在普通 Agent 会话描述异常
-> Agent 根据 skill 判断需要创建异常追踪
-> Agent 调用 prepare_pet_abnormal_symptom_creation
-> 后端创建 agent_confirmation_tasks.task_kind=abnormal_symptom_creation
-> 前端展示确认卡，用户授权
-> 后端 approve/stream 创建 pet_events.health/abnormal_symptom
-> 后端事务创建 abnormal_episode 和初始 agent_proactive_followups.status=planning
-> 当前 ai_chat_sessions 绑定 chat_context_kind=abnormal_episode_followup、abnormal_episode_id、agent_followup_id
-> 后端把 resolved_event_id、episode_id、agent_followup_id、next_followup_due_at 作为已完成事实交给同一 Agent Runtime
-> Agent 自然回复用户已开始追踪和后续会提醒更新实际情况
```

| 步骤 | 责任层 | 产物 |
|---|---|---|
| 异常事件落库 | Pet application / infrastructure | `pet_events.health/abnormal_symptom` |
| Agent 异常创建确认 | Runtime tool + confirmation task repository | `prepare_pet_abnormal_symptom_creation` 只创建 `agent_confirmation_tasks.abnormal_symptom_creation`，授权前不写事实账本 |
| Agent 授权创建异常 | HTTP 授权端点 + Pet application / infrastructure + AI session repository | `approve/stream` 创建 `health/abnormal_symptom`、`abnormal_episode`、初始 `planning`，并把当前 session 绑定到同一 episode/followup |
| episode 创建 | Pet application / infrastructure | `abnormal_episodes.status=open` |
| Agent planning 触发 | Pet infrastructure + backend planner job | `agent_proactive_followups.status=planning`；`agent_followup_planner` 周期领取并运行 Agent Runtime |
...
| 类型 | 什么时候创建 | 职责 | 禁止承担 |
|---|---|---|---|
| Agent skill | 需要多步理解、追问策略、规划规则、跨 turn 一致行为、自然语言归纳时 | 异常追踪 planning、首条追问生成、根据用户反馈决定继续观察/建议就医/结束追踪、判断普通聊天描述是否需要整理为异常创建草稿、把自然语言整理成待确认异常或追加观察、基于 `occurred_at/last_observed_at/now_at` 评估信息断层、结合身份/年龄上下文输出 `time_decision` 解释追问时间 | 直接写数据库、定时调度、绕过工具读取事实、把未确认用户话术写成事实 |
| Agent tool | Agent 需要访问系统事实或执行受控副作用时 | 读取 episode facts、读取 quick facts、读取饮食/储物柜线索、准备经用户确认的异常创建草稿、准备经用户确认的追加观察草稿、提交追踪计划结果 | 自行决定追踪节奏、隐藏多步业务策略、替代 skill 推理、返回和职责无关的综合摘要 |
...
| 需要决定普通聊天是否进入异常追踪 | 模型结合用户文字、宠物上下文和风险边界判断是否发起确认卡 | “昨天精神不好”可以整理为 `abnormal_symptom_creation` 待确认草稿，等待用户授权 |
...
| 需要执行受控写入 | 输入字段明确、权限明确、可验证、可回滚或可审计 | 准备用户确认后的 `abnormal_symptom_creation`、提交用户确认后的 `symptom_followup` |
| 需要把模型输出交给后端校验入库 | Tool 只传递候选结果，后端负责权限、字段和状态机；不按规则覆盖模型追问时间 | 保存 `agent_proactive_followup` 的 `due_at/message/rationale/time_decision/actions` |
...
| 用户普通聊天描述异常后准备创建异常追踪 | Tool：`prepare_pet_abnormal_symptom_creation` 创建确认任务，授权前不写 `pet_events` |
| 用户确认创建异常追踪 | HTTP 授权端点调用 application service，创建 `health/abnormal_symptom`、`abnormal_episode`、初始 `planning` 并绑定当前 session |
...
| Agent 授权创建异常后回复用户 | 后端把已创建事件、episode、followup、下一次提醒时间作为系统事实交给同一 Agent Runtime，由模型自然回复 |

#### 5.3.6 用户响应后的状态处理

| 用户动作 | 后端处理 | Agent 后续 |
|---|---|---|
| 点击“更新情况”并提交追加观察 | 写 `health/symptom_followup`，更新 episode `last_observed_at/latest_event_id`，resolve 当前 hint/followup；按 `condition_change` 更新为 `recovering/watching/escalated` | 基于新事实重新 planning；好转分支带 `mark_recovered`，持续分支继续追踪，加重分支带 `book_clinic` |
| 点击“问问毛球”并进入聊天 | 打开同一 `abnormal_episode_followup` 后台追踪 session，并把 `session_visibility` 从 `background` 升级为 `visible`，写入 `activated_at`；首屏展示异常卡片和 Agent 追问 | 用户反馈后，Agent 生成待确认追加观察；确认后写入 |
| 标记“已经好了” | 写 `health/abnormal_recovery`，episode `status=recovered`，清空 `next_followup_due_at/last_followup_plan_id` | 取消所有未到期/已到期追踪计划，保留历史供 HIS 参考 |
...
确认任务的流式可见结果必须保持单一数据流：模型在同一轮里先输出自由文本、随后调用 `prepare_*` 确认类工具时，Runtime 可以保留原始内部事件和工具调用，但 SSE Projector 不把该轮自由文本投给前端；用户可见主结果是 `confirmation_task` 授权卡，授权卡 `preview.note` 展示待写入内容。用户确认后，后端真实写入完成，再由同一 Agent Runtime 基于写入结果自然回复用户。
...
| Task 2：异常事件触发后台 planning | 创建/追加异常后产生 `planning` 计划意图，后台 planner job 领取后由 Agent Runtime 保存为 `scheduled` plan | pet contract + AI contract | pet infrastructure、agent_followup_planner、Runtime tool registry | `cargo test -p maohuoban_rust --test pet_contract agent_proactive_followup -- --nocapture --test-threads=1`；`cargo test -p maohuoban_rust --test ai_contract planner_run_once_saves_dynamic_plan_for_pending_abnormal_followup -- --nocapture --test-threads=1` |
...
| Task 6.2：Agent 聊天内创建异常追踪 | 普通 Agent 会话中模型可准备异常创建确认卡；用户授权后后端创建异常事件、episode 和初始 planning，并把当前 session 绑定为同一 abnormal episode 追踪上下文；后端把 planning 事实交给同一 Agent Runtime 自然回复用户 | AI chat runtime contract | Runtime tool schema、confirmation task kind、abnormal creation provider、confirmation approve stream、session repository | `cargo test -p maohuoban_rust --test ai_contract chat_stream_runtime_tools:: -- --nocapture --test-threads=1` |
| Task 6.3：确认卡前自由文本抑制 | 同一轮模型先输出 `content` delta 后调用确认类工具时，前端 SSE 只收到授权卡，不先收到 `answer_delta`；普通回答仍在模型完成后正常输出 | AI chat runtime stream contract | Runtime SSE Projector | `cargo test -p maohuoban_rust --test ai_contract chat_stream_runtime_tools::ai_chat_stream_suppresses_model_delta_before_confirmation_task -- --exact --nocapture` |
...
| Agent 上下文恢复合同 | `cargo test -p maohuoban_rust --test ai_contract abnormal_followup_second_turn_restores_agent_context_from_session -- --nocapture`；必须同时断言同一 `chat_session_id`、session 持久化上下文字段、Runtime 写入上下文、受控写入 tool 上下文和 provider workflow 注入 |
...
| Agent 确认卡流式边界合同 | `cargo test -p maohuoban_rust --test ai_contract chat_stream_runtime_tools::ai_chat_stream_suppresses_model_delta_before_confirmation_task -- --exact --nocapture`；必须覆盖同轮模型先输出自由文本再调用 `prepare_pet_observation_write` 时，SSE 不发送 `answer_delta`，只发送包含 `preview.note` 的 `confirmation_task` |
| Agent 异常创建合同 | `cargo test -p maohuoban_rust --test ai_contract chat_stream_runtime_tools:: -- --nocapture --test-threads=1`；必须覆盖 `prepare_pet_abnormal_symptom_creation` 确认卡、授权前不写 `pet_events`、授权后创建 `health/abnormal_symptom`、`abnormal_episode`、初始 `agent_proactive_followups.status=planning`、当前 session 绑定 `chat_context_kind=abnormal_episode_followup` 并由同一 Agent Runtime 回复 |
...
| Agent planning 分支动作合同 | `cargo test -p maohuoban_rust --test ai_contract abnormal_followup_plan_can_save_branch_actions_from_session_context -- --nocapture --test-threads=1`，tool schema 和 application service 必须允许 `mark_recovered/book_clinic` 这类受控分支动作 |
...
| 后台动态 planning job 合同 | `cargo test -p maohuoban_rust --test ai_contract planner_run_once_saves_dynamic_plan_for_pending_abnormal_followup -- --nocapture --test-threads=1`，创建异常后的 `planning` followup 必须由后台 Agent Runtime 读取多源事实并通过受控 tool 保存为 `scheduled` |
| 首条主动追问消息合同 | `cargo test -p maohuoban_rust --test pet_contract scheduler_run_once_writes_first_agent_followup_message -- --nocapture --test-threads=1`，到期投影必须创建/复用同一异常追踪会话并写入一条 assistant 主动追问，重复运行保持幂等 |
| 终止/分支状态机合同 | `cargo test -p maohuoban_rust --test pet_contract agent_proactive_followup -- --nocapture --test-threads=1`，覆盖恢复/删除清空追踪投影、好转进入 `recovering`、持续进入 `watching`、加重进入 `escalated` 和下一轮 planning 动作 |
...
| 确认前文本不入可见流 | 进入确认任务的同一轮模型自由文本不得先作为 `answer_delta` 投给前端；授权前用户可见内容以确认卡 `preview` 为准，授权后再由同一 Agent Runtime 生成回复 |
...
➡️ 用户预约
➡️ 系统生成诊前资料包
➡️ Web HIS 真实工作台接收 App 预约单并展示诊前资料包
➡️ HIS 发布医院病历
➡️ App 病历记录出现回流病历
```
...
| 4 | App 预约医院链路接入 Web HIS 工作台 | 已具备真实预约创建、取消预约、HIS 工作台过滤取消预约和后端 message Toast |
| 5 | Web HIS 接收诊前资料包 | 在真实预约队列上展示医生侧诊前事实快照 |
| 6 | HIS 发布病历回流 App | 完成医院病历闭环 |
| 7 | Reminder / Notification 独立模块 | 在提醒边界明确后独立建设 |
