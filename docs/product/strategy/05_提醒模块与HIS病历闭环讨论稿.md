...
| 4. Agent 主动追踪计划 | 已具备初始 proactive followup 表、异常创建后默认追踪计划、due_at 到期投影、真实 scheduler 后台任务、追加后下一轮默认重规划、恢复/删除后的取消或 resolve 状态机 | 补齐 Agent skill 动态 planning | 部分验证 |
...
当前仍未完成的是 Agent skill 的动态 planning。
