# zavora-ai/adk-rust: Rust Agent Development Kit (ADK-Rust): Build AI agents in Rust with modular components for models, tools, memory, realtime voice, and more. ADK-Rust is a flexible framework for developing AI agents with simplicity and power. Model-agnostic, deployment-agnostic, optimized for frontier AI models. Includes support for real-time voice agents.

- 来源链接：https://github.com/zavora-ai/adk-rust
- 浏览器最终地址：https://github.com/zavora-ai/adk-rust
- 原始收集：已删除的资料草稿
- 保存时间：2026-06-28
- 访问状态：success
- 提取方式：浏览器可见正文提取
- 页面描述：Rust Agent Development Kit (ADK-Rust): Build AI agents in Rust with modular components for models, tools, memory, realtime voice, and more. ADK-Rust is a flexible framework for developing AI agents with simplicity and power. Model-agnostic, deployment-agnostic, optimized for frontier AI models. Includes support for real-time voice agents. - zavora-ai/adk-rust

---

## 原始备注

ADK-Rust GitHub 链接，原文写作 GitHub zavora-ai/adk-rust
...
Latest commit
jkmaina
Merge pull request #398 from morlay/deepseek-client-fix
b1d88f8
 · 
History
949 Commits
...
feat(publish): shell-agnostic publishing via cargo xtask


.config

v0.4.0 — Framework focus, performance, DX (#207)


.github

docs: add example UI screenshots (light/dark) for issue & PR


.skills/skills

chore: bump workspace to 1.1.0
...
fix(adk-anthropic): replace blanket From<AsRef<str>> impl for Message…
...
feat: v1.0.0 release hardening, security docs, podcast episode 2


adk-bench

fix(bench): gate memory tests behind cfg(linux/macos) for Windows CI
...
feat(devtools): add adk-devtools crate (coding agent Phase 1)
...
docs: update all version references to 1.0.0 across 52 files


adk-gemini

Merge pull request #389 from hiratara/fix/rename-gemini-function-decl…
...
fix(hooks): make lefthook the single hook manager and pass its gates


adk-memory

feat(memory): token-based recall for the knowledge graph
...
feat(realtime): affective dialogue config for Gemini Live
...
fix(sandbox): repair wasm feature build and prove timeout isolation
...
feat(tool): knowledge-graph memory curation tools (remember/relate)


assets

v0.3.0: release build — workspace cleanup, studio codegen fixes, vers…


awp-types

fix: remove duplicate homepage key in awp-types and adk-awp Cargo.toml
...
feat(goal): durable/resumable /goal + coding_goal example
...
ci: add CodeQL config to exclude minified vendor JS


.env.example

chore: clean up .env.example to match actual project usage (#255)


.envrc

chore: synchronized environment with origin/main and optimized settings


.gitignore

examples: add harness_pattern (trait-based agent execution)


AGENTS.md

feat(eval): add competitive-parity capabilities
...
feat(cli,examples): add 'adk-rust code' CLI + coding_agent example
...
feat(anthropic): add adk-anthropic crate — dedicated Anthropic API cl…
...
docs: replace 30 stale roadmap docs with authoritative ROADMAP.md
...
chore: promote all Beta crates to Stable, fix tar vuln in mcp_elicita…


devenv.lock

chore(env): Upgrade devenv to v2.0.6 and improve local setup safety (#…
...
chore(hooks): cover xtask in gate globs, shellcheck publish.sh
...
🚀 v1.0.0 Released! First stable release — 39 crates, 130K+ downloads in 6 months, semver stability commitment, all former Beta crates promoted to Stable. Plus: adk-bench benchmarking framework (4.6× faster cold start vs Python), authoritative ROADMAP.md, and security hardening. See CHANGELOG for full details.
...
▶️ Watch on YouTube — 10 min 12 sec · Hosts: James (Fenrir) & Ada (Kore) · Video with slides
...
Or pick a template: --template tools | rag | api | openai | a2a | graph | realtime | sequential | parallel | loop. Combine with addons: --addon telemetry | auth | sessions | memory | mcp | guardrails | eval | browser | server. Run cargo adk templates and cargo adk addons for the full list, and see Quick Start for details.
...
goal — Codex/Hermes-style autonomous loop (plan → act → verify against a --until command), durable & resumable via an on-disk checkpoint.
ultracode — Claude Code-style fan-out to parallel correctness/edge-case/style reviewers on adk-graph, revising until they approve.

Examples: coding_agent, coding_graph, coding_goal. Full guide: docs/official_docs/coding-agent.
...
adk-rust.com/
Topics
realtime realtime-audio adk google-adk adk-google adk-rust adk-agent adk-model adk-artifact agent-developer-kit google-adk-rust adk-tool adk-memory adk-cli adk-server openai-adk realtime-adk
Resources
Readme
License
View license
