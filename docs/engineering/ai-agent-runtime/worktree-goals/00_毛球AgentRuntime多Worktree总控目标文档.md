...
| 项 | 结论 |
|---|---|
| 最终形态 | 自有 Agent Runtime SDK + AgentSession Workbench + Capability Catalog + 可插拔 Loop Engine + 领域安全能力层 + 多端 Adapter + Eval / Replay 基础设施 |
| 首期 Agent 形态 | 单主 Agent + 能力工作台 + 公共宠物能力 + 用户宠物私域工具；预留多 Agent 接口，首期只注册 `main_pet_care_agent` |
| 框架策略 | Rig 用于 `LoopEngine` adapter POC；ADK-Rust 借鉴工具元数据；Anda 借鉴工具发现、模型路由、上下文压缩；Pi 借鉴 SDK / Adapter 分层 |
...
|---|---|---|---|
| WT-01 | `01_WT01_Runtime契约与LoopEngine目标文档.md` | Runtime 核心契约、事件、LoopEngine trait、AgentSession Workbench、AgentDefinition、ContextPack、CapabilityCatalog | `maohuoban-ai-domain/src/ai/model/runtime*`、`maohuoban-ai-application/src/ai/runtime*` |
| WT-02 | `02_WT02_ToolGateway与PolicyGuard目标文档.md` | 工具元数据、工具发现、策略裁决、确认事件、工具进度文案、公共 / 私域工具分组 | `maohuoban-ai-application/src/ai/tools*`、`maohuoban-ai-application/src/ai/policy*` |
...
iOS / History / IM / Replay / CLI
  ...
  -> AgentSession Workbench / AgentSessionRuntime
  -> AgentDefinition / CapabilityCatalog / ContextPack / MemoryPack
  -> LoopEngine Adapter
...
| Agent 能力工作台 | `AgentSessionWorkbench` |
...
| Workbench 契约 | `AgentDefinition`、`CapabilityCatalog`、`ContextPack`、`MemoryPack` 属于 WT-01 契约，其他 worktree 只消费 |
...
| 无宠物用户 | 没有授权宠物或 selected pet 时，公共宠物咨询和 App 帮助仍可进入 AgentSession Workbench；只是不加载私域宠物事实 |
| Gate 职责 | 入口 Gate 只做硬安全和成本限制裁决，不再用 `context_loaded=false` 表达是否进入 Agent |
...
| Workbench 演化成新规则机 | 能力目录只说明模型可用能力和工具，不预先替模型固定回答路线 |
