...
| 后端基线 | 已完成首页聚合和宠物事件接口基线 | 新增 home domain/application/http crates；新增 pet domain/application/infrastructure/http crates；`home_contract.rs` 覆盖普通用户、空态、商家态、当前用户真实宠物聚合；`pet_contract.rs` 覆盖宠物档案、事件追加和时间线 |
...
| 3. 宠物事件底座 | 已完成接口基线 | 已新增宠物、事件、窝次、关系、证据快照数据库基线；已实现宠物档案创建、事件追加、时间线读取 HTTP 契约；窝次和关系树应用用例后续迭代 |
...
| 首页接口契约 | `maohuoban-rust/tests/home_contract.rs` | 已完成基线，覆盖无上下文 seed、当前用户无宠物空态、创建宠物后真实首页聚合 |
