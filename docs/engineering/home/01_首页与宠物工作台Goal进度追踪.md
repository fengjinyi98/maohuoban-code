...
| 后端基线 | 已完成首页聚合、宠物事件接口、交易导入接口、商家追溯应用基线、商家宠物状态列表接口、商家新增宠物接口、可售状态发布接口、窝次详情接口、事件详情接口、同城医院列表、医院预约接口、首页关系推荐基线和多宠切换闭环 | 新增 home domain/application/http crates；新增 pet domain/application/infrastructure/http crates；新增 samecity domain/application/infrastructure/http crates；新增 recommendation domain/application/infrastructure crates；`home_contract.rs` 覆盖普通用户、空态、seed 商家态、当前用户真实宠物聚合、当前用户真实事件派生今日照护与提醒、当前用户认证商家真实窝次追溯聚合、显式同窝关系伙伴推荐、新用户公开 UGC 空态内容、`selected_pet_id` 多宠切换和跨用户宠物 ID 回退；`pet_contract.rs` 覆盖宠物档案、事件追加、交易导入、时间线、事件详情、商家在管宠物状态筛选、认证商家新增在管宠物、可售状态发布和窝次详情追溯上下文；`samecity_contract.rs` 覆盖城市医院列表和当前用户宠物预约 |
| iOS 首页 | 已完成首页骨架、当前用户上下文、多宠切换器、动作路由、提醒入口、时间线入口、Pet 写入基线、交易导入页、事件详情页、商家宠物列表目标页、商家新增宠物页、可售状态发布页、窝次详情页、商家待补记录入口和医院预约页 | `HomeRootScreen` 已消费 `HomeDashboardSnapshot`，`AuthRootView` 将当前 user id 传入首页 Store，`HomePetSwitcherSection` 使用后端宠物 ID 做稳定 identity 并通过 `selected_pet_id` 刷新首页，`.createPet` / `.recordDaily` / `.recordHealth` / `.importTradePet` / `.timelineEvent` 已接入 `Features/Pet`，首页健康提醒和普通时间线行进入 `.timelineEvent`，`.merchantPets` / `.addMerchantPet` / `.publishAvailableStatus` / `.merchantLitter` / `.merchantTask` 已接入 `Features/Merchant` 真实读写流程，商家提醒进入 `.merchantTask`，`.bookHospital` 已接入 `Features/SameCity` 的 `HospitalBookingScreen`，登录后新用户空态 UI 契约通过 |
...
| 5. 关系推荐与 UGC 空态 | 已完成后端推荐基线 | recommendation 三层 crate 已接入首页聚合；显式同窝关系生成今日伙伴；新用户空态读取真实公开事件作为辅助内容 |
| 6. 多宠切换闭环 | 已完成前后端实现和门禁 | `/api/v1/home/dashboard?selected_pet_id={pet_id}` 已接入；后端只在当前用户宠物集合内选择当前宠物；iOS 首页切换器已按当前选择刷新；`cargo test --test home_contract`、`cargo test --workspace`、`cargo clippy --workspace --all-targets`、iOS 单元测试、iOS Debug build 和 `git diff --check` 已通过 |
| 7. 提醒与时间线入口闭环 | 已完成 iOS 实现和门禁 | 健康提醒进入事件详情，商家提醒进入待补记录，普通时间线行进入事件详情；`HomeRouteTests`、`maohuobanTests`、iOS Debug build 和 `git diff --check` 已通过 |
| 8. 端到端验证 | 已完成当前首页主链路门禁 | 推荐切片、多宠切换切片和提醒 / 时间线入口切片 Rust / iOS 门禁均已通过 |
...
| 后端分层可扩展 | `maohuoban-home-domain`、`maohuoban-home-application`、`maohuoban-home-http`、`maohuoban-recommendation-domain`、`maohuoban-recommendation-application`、`maohuoban-recommendation-infrastructure` | 已完成首页和推荐基线 |
| 首页接口契约 | `maohuoban-rust/tests/home_contract.rs` | 已通过，10 tests，覆盖无上下文 seed、当前用户无宠物空态、创建宠物后真实首页聚合、真实事件派生今日照护与提醒、认证商家真实窝次追溯聚合、显式同窝关系伙伴推荐、新用户公开 UGC 空态内容、`selected_pet_id` 多宠切换和跨用户宠物 ID 回退 |
...
| 普通用户首页 | iOS 首页渲染宠物主卡、宠物切换器、今日照护、快捷动作、伙伴、时间线；后端可读取当前用户宠物档案，并从真实 `PetEvent` 派生食欲、体重、驱虫提醒和驱虫 / 疫苗时间线类型；创建宠物、导入交易宠物、记录日常、健康记录、医院预约进入真实写入流程；健康提醒和最近时间线事件进入真实事件详情页 | 已完成写入、交易导入、医院预约、详情读取、多宠切换、提醒入口和时间线入口闭环 |
| 新用户空态 | 无宠物时展示创建宠物和辅助内容入口；创建宠物主操作进入真实 `PetCreateScreen`，交易导入入口进入真实 `PetTradeImportScreen`；后端可从真实公开 `PetEvent` 生成辅助 UGC 内容；UI 测试覆盖登录后键盘消失和创建宠物入口 | 已完成当前用户上下文和公开内容推荐基线 |
| 首页关系推荐 | 当前宠物有显式 `same_litter` 关系时展示同窝伙伴；无显式关系时可降级读取公开宠物事件；推荐失败不阻断首页主快照 | 已完成后端契约，`cargo test --test home_contract` 10 tests 通过 |
...
| iOS 首页提醒路由目标测试 | `xcodebuild test -project maohuoban/maohuoban.xcodeproj -scheme maohuoban -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=27.0' -configuration Debug -only-testing:maohuobanTests/HomeRouteTests` | 已通过，5 tests，覆盖健康提醒进入事件详情、商家提醒进入待补记录、缺失商家上下文无路由、商家待办路由和医院预约路由上下文 |
| iOS 单元测试 | `xcodebuild test -project maohuoban/maohuoban.xcodeproj -scheme maohuoban -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=27.0' -configuration Debug -only-testing:maohuobanTests` | 已通过，45 tests，覆盖 DTO 解码、普通用户真实事件派生 payload、Store loading -> loaded / failed、多宠切换 selected pet 参数传递、首页仓库 query 契约、动作路由上下文、提醒路由上下文、缺失商家上下文兜底、Pet 写入请求头、交易导入请求与状态流、`PetWriteStore` / `PetEventDetailStore` 状态流、商家宠物列表 / 新增 / 可售发布 / 窝次详情请求契约、`MerchantPetsStore`、`MerchantPetCreateStore`、`MerchantAvailableStatusStore`、`MerchantLitterDetailStore`、`SameCityRepository` 和 `HospitalBookingStore` 状态流；`HomeRouteTests` 覆盖健康提醒、商家提醒、商家待办路由和医院预约路由上下文 |
| Rust 测试 | `cargo test --workspace` | 已通过，覆盖 recommendation domain/application/infrastructure 新 crate 和 `home_contract.rs` 10 tests |
...
|---|---|
| 首页一句定调 | 以宠物为主体的信任工作台 |
| 普通用户首屏 | 当前宠物主卡、宠物切换器、今日照护、快捷动作、今日伙伴、最近时间线 |
...
| iOS | 首页前端只消费 `HomeDashboardSnapshot`，不在 View 中写副作用；多宠切换通过 `HomePetSwitcherSection` 转发事件，由 `HomeDashboardStore` 带 `selected_pet_id` 重新读取快照；动作通过 `HomeActionRouteResolver` 转为 `HomeRoute`；提醒通过 `HomeReminderRouteResolver` 转为事件详情或商家待办；创建宠物、交易导入、记录事件和事件详情由 `Features/Pet` 承接；商家宠物状态列表、待补记录入口、新增宠物、发布可售状态和窝次详情由 `Features/Merchant` 承接；医院预约由 `Features/SameCity` 承接；UI 测试通过 `-MHB_BACKEND_BASE_URL` 启动参数固定后端地址 |
| 后端 | 首页聚合服务只做读模型编排，深层规则由 pet/merchant/samecity/reminder/recommendation 域承担；`selected_pet_id` 只在当前用户宠物集合内生效，非法或越权 ID 回退默认宠物；recommendation 域优先读取显式宠物关系，再降级到公开事件 |
