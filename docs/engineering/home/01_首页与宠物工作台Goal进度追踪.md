...
| iOS | 首页前端只消费 `HomeDashboardSnapshot`，不在 View 中写副作用；动作通过 `HomeActionRouteResolver` 转为 `HomeRoute`；创建宠物与记录事件由 `Features/Pet` 承接；商家宠物状态列表由 `Features/Merchant` 承接；UI 测试通过 `-MHB_BACKEND_BASE_URL` 启动参数固定后端地址 |
