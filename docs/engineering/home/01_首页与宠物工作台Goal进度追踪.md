...
| iOS 首页 | 已完成首页骨架、当前用户上下文、动作路由、创建宠物与记录事件写入基线 | `HomeRootScreen` 已消费 `HomeDashboardSnapshot`，`AuthRootView` 将当前 user id 传入首页 Store，`.createPet` / `.recordDaily` / `.recordHealth` 已接入 `Features/Pet` 真实写入流程，登录后新用户空态 UI 契约通过 |
...
| 4. iOS 首页骨架 | 已完成当前用户上下文、单测 target、动作路由和 Pet 写入基线 | `HomeRouteDestinationScreen` 已将创建宠物、记录日常、健康记录替换为真实 Pet 写入页面；商家、医院、交易和窝次详情继续等待对应业务模块接管 |
...
| iOS 单元测试 | `xcodebuild test -project maohuoban/maohuoban.xcodeproj -scheme maohuoban -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=27.0' -configuration Debug -only-testing:maohuobanTests` | 已通过，12 tests，覆盖 DTO 解码、Store loading -> loaded / failed、动作路由上下文、缺失商家上下文兜底、Pet 写入请求头和 `PetWriteStore` 状态流 |
...
| iOS | 首页前端只消费 `HomeDashboardSnapshot`，不在 View 中写副作用；动作通过 `HomeActionRouteResolver` 转为 `HomeRoute`；创建宠物与记录事件由 `Features/Pet` 承接；UI 测试通过 `-MHB_BACKEND_BASE_URL` 启动参数固定后端地址 |
