...
| 后端基线 | 已完成首页聚合和宠物事件接口基线 | 新增 home domain/application/http crates；新增 pet domain/application/infrastructure/http crates；`home_contract.rs` 覆盖普通用户、空态、商家态；`pet_contract.rs` 覆盖宠物档案、事件追加和时间线 |
