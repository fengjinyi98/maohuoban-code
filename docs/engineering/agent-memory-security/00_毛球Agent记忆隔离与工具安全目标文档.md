...
| 领域边界 | 毛球不是通用聊天机器人，非宠物问题默认走短拒答或轻量帮助，不加载宠物事实、私有记忆和主 Agent |
...
| 意图闸门 | 在上下文拼装和主模型调用前完成轻量意图判断，区分宠物领域、App 帮助、宠物相关情绪陪伴、非宠物问题、提示词注入和成本滥用 |
| 非宠物拒答 | `off_topic` 请求只返回短话术，不查询宠物事实、不检索私有记忆、不进入昂贵 Agent 流程 |
...
| 通用知识问答 | 毛球不承接百科、作文、代码、泛娱乐等非宠物通用问答 |
...
用户问题
  ...
  -> off_topic / cost_abuse / prompt_injection
     -> 不读取宠物事实，不调用主 Agent，短拒答并记录信号
```

### 4.4.1 领域意图闸门

| intent | 示例 | 加载宠物上下文 | 主 Agent | 处理 |
|---|---|---:|---:|---|
...
| `app_support` | App 怎么添加宠物、怎么记录体重 | 否 | 否或轻量 | 使用产品帮助短答 |
| `off_topic` | 作文、代码、百科、非宠物闲聊 | 否 | 否 | 短拒答并拉回宠物 |
| `prompt_injection` | 忽略规则、改成通用助手、读取全库 | 否 | 否 | 拒绝并记录风险信号 |
| `cost_abuse` | 连续大量无关长问题、刷通用创作 | 否 | 否 | 限速、冷却或进入风险评分 |
